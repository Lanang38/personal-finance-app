...
export function SuggestionPanel({
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm">
      ...
      {isLoading ? (
        <p className="text-sm text-slate-400 py-4">Menganalisis data kamu...</p>
        ...
        <div className="divide-y divide-slate-100">
          {visibleSuggestions.map((suggestion) => (
            ...
            >
              ...
              />
              <p className="text-sm text-slate-600 flex-1">{suggestion.text}</p>

              {suggestion.action && (
                <button
                  type="button"
                  onClick={() => navigate(suggestion.action!.route)}
                  className="flex items-center gap-1 text-xs font-semibold text-brand-purple bg-brand-purple/10 px-3 py-1.5 rounded-full whitespace-nowrap shrink-0"
                >
