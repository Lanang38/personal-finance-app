  ...
  async (req: Request, res: Response) => {
    ...
    if (type === 'expense') {
      ...
      if (amount > currentBalance) {
        throw new AppError(
          'Saldo akun tidak mencukupi untuk transaksi ini',
          400,
        );
      }
    }
  ...
  async (req: Request, res: Response) => {
    ...
    if (accountId !== undefined)
      transaction.accountId =
        accountId as unknown as typeof transaction.accountId;
    if (categoryId !== undefined)
      transaction.categoryId =
        categoryId as unknown as typeof transaction.categoryId;
